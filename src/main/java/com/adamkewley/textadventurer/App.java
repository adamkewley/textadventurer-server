...
import com.adamkewley.textadventurer.configuration.ResolvedApplicationConfiguration;
import com.adamkewley.textadventurer.configuration.UnresolvedApplicationConfiguration;
import com.adamkewley.textadventurer.dao.GamesDAO;
import com.adamkewley.textadventurer.dao.InMemoryGamesDAO;
import com.adamkewley.textadventurer.resources.GameSocket;
import com.adamkewley.textadventurer.resources.GamesResource;
...
public class App extends NanoWSD {
    ...
    public Response serveHttp(IHTTPSession session) {
        ...
        if (uri.equals("/") || uri.endsWith("index.html")) {
            String index;
            try {
                index = new String(Files.readAllBytes(Paths.get(getClass().getResource("/index.html").toURI())));
            } catch (IOException e) {
                e.printStackTrace();
                index = "ERROR";
            ...
            return newFixedLengthResponse(Response.Status.OK, "text/html", index);
        } else if (uri.contains("play-game.html")) {
            String index;
            try {
                index = new String(Files.readAllBytes(Paths.get(getClass().getResource("/play-game.html").toURI())));
            } catch (IOException e) {
                e.printStackTrace();
                index = "ERROR";
            } catch (URISyntaxException e) {
            ...
            return newFixedLengthResponse(Response.Status.OK, "text/html", index);
        } else if (uri.contains(GAMES_API_PATH)) {
